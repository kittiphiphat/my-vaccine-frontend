...
function generateTimeOptions(serviceStartTime, serviceEndTime, intervalMinutes = 15) {
  ...
  if (endTimeInMinutes <= startTimeInMinutes) {
    console.warn(`Invalid service time range: start=${serviceStartTime}, end=${serviceEndTime}`);
    return [];
  }
  ...
  console.log('Generated time options:', times.map(opt => opt.value));
  return times;
}
...
function filterAvailableTimes(allTimes, allSlots, currentId, selectedVaccineId, currentStartTime, currentEndTime) {
  if (!allSlots || allSlots.length === 0 || !selectedVaccineId) return allTimes;

  const occupiedTimes = new Set();
  allSlots.forEach((slot) => {
    if (slot.id === currentId) return; // Skip the slot being edited
    if (slot.attributes.vaccine?.data?.id !== selectedVaccineId) return; // Skip if vaccine doesn't match
    ...
    if (slotStart && slotEnd) {
      ...
      // Include all time points from start to end (inclusive) in 15-minute intervals
  ...
  // Filter out occupied times, but include current slot's start and end times
  const filtered = allTimes.filter((time) => {
    const isOccupied = occupiedTimes.has(time.value);
    const isCurrentStart = currentStartTime && time.value === currentStartTime;
    const isCurrentEnd = currentEndTime && time.value === currentEndTime;
    return !isOccupied || isCurrentStart || isCurrentEnd;
  });

  console.log('Filtered available times:', filtered.map(opt => opt.value));
  return filtered.sort((a, b) => timeToMinutes(a.value) - timeToMinutes(b.value));
}
...
export default function VaccineTimeSlotForm({
  ...
  const [startTime, setStartTime] = useState(
    initialData.startTime ? { value: initialData.startTime.slice(0, 5), label: initialData.startTime.slice(0, 5) } : null
  );
  const [endTime, setEndTime] = useState(
    initialData.endTime ? { value: initialData.endTime.slice(0, 5), label: initialData.endTime.slice(0, 5) } : null
  );
  ...
  useEffect(() => {
    async function fetchVaccines() {
      try {
        ...
        setVaccines(fetchedVaccines.map(vaccine => ({
          value: vaccine.id,
        ...
        MySwal.fire({
          ...
          text: error.message === 'Failed to fetch vaccines'
  ...
  const allTimeOptions = useMemo(() => {
    const startTime = selectedVaccine?.serviceStartTime || initialData.vaccine?.data?.attributes?.serviceStartTime?.slice(0, 5) || '08:00';
    const endTime = selectedVaccine?.serviceEndTime || initialData.vaccine?.data?.attributes?.serviceEndTime?.slice(0, 5) || '17:00';
    const times = generateTimeOptions(startTime, endTime);
    console.log('All time options:', times.map(opt => opt.value));
    return times;
  }, [selectedVaccine, initialData.vaccine?.data?.attributes?.serviceStartTime, initialData.vaccine?.data?.attributes?.serviceEndTime]);

  const timeOptions = useMemo(() => {
    const vaccineId = selectedVaccine?.value || initialData.vaccine?.data?.id;
    if (!vaccineId) return allTimeOptions;
    const currentStart = initialData.startTime?.slice(0, 5);
    const currentEnd = initialData.endTime?.slice(0, 5);
    const filteredTimes = filterAvailableTimes(allTimeOptions, allSlots, initialData.id, vaccineId, currentStart, currentEnd);
    console.log('Filtered time options:', filteredTimes.map(opt => opt.value));
    return filteredTimes;
  }, [allTimeOptions, allSlots, initialData.id, initialData.startTime, initialData.endTime, selectedVaccine, initialData.vaccine?.data?.id]);

  const endTimeOptions = useMemo(() => {
    if (!startTime) return [];
    const startMinutes = timeToMinutes(startTime.value);
    const vaccineId = selectedVaccine?.value || initialData.vaccine?.data?.id;
    const filteredSlots = allSlots.filter(slot => slot.attributes.vaccine?.data?.id === vaccineId && slot.id !== initialData.id);

    const availableEndTimes = timeOptions.filter((opt) => {
      const optMinutes = timeToMinutes(opt.value);
      if (optMinutes <= startMinutes) return false; // Ensure end time is after start time

      const tempEndTime = formatTimeToStrapi({ value: opt.value });
      const startTimeStr = formatTimeToStrapi(startTime);
      const { isOverlap } = isTimeOverlap(startTimeStr, tempEndTime, filteredSlots, initialData.id);

      // Check if the entire range from startTime to opt.value overlaps with any existing slot
      return !isOverlap;
    });

    console.log('End time options:', availableEndTimes.map(opt => opt.value));
    return availableEndTimes;
  }, [startTime, timeOptions, allSlots, initialData.id, selectedVaccine, initialData.vaccine?.data?.id, isTimeOverlap]);
  ...
  async function handleSubmit(e) {
    ...
    if (!selectedVaccine || !startTime || !endTime || quota < 1) {
      await MySwal.fire({
        title: 'แจ้งเตือน',
        text: 'กรุณาเลือกวัคซีน, เวลาเริ่ม, เวลาสิ้นสุด และจำนวนโควต้าที่มากกว่า 0',
        icon: 'warning',
        customClass: {
          popup: 'bg-white/90 backdrop-blur-lg rounded-2xl shadow-lg border border-[#30266D]/50 p-6',
          title: 'text-lg font-bold text-[#30266D] mb-3',
    ...
    const vaccineId = selectedVaccine?.value || initialData.vaccine?.data?.id;
    const filteredSlots = allSlots.filter(slot => slot.attributes.vaccine?.data?.id === vaccineId);
    const { isOverlap, conflicts } = isTimeOverlap(formatTimeToStrapi(startTime), formatTimeToStrapi(endTime), filteredSlots, initialData?.id);
    if (isOverlap) {
      ...
      await MySwal.fire({
        title: 'เกิดข้อผิดพลาด',
        text: `ไม่สามารถเลือกช่วงเวลานี้ได้ มีช่วงเวลา ${conflictMessage} อยู่แล้ว`,
        icon: 'error',
        customClass: {
          popup: 'bg-white/90 backdrop-blur-lg rounded-2xl shadow-lg border border-[#30266D]/50 p-6',
          title: 'text-lg font-bold text-[#30266D] mb-3',
    ...
    const confirm = await MySwal.fire({
      ...
      text: 'คุณต้องการบันทึกการแก้ไขช่วงเวลาให้บริการนี้หรือไม่?',
      icon: 'question',
    ...
    const payload = {
      data: {
        ...
        endTime: filterTimeToStrapi(endTime),
        quota,
    ...
    try {
      const url = `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/vaccine-time-slots/${initialData.id}`;
      ...
      if (!res.ok) {
        ...
        throw new Error(result.error?.message || 'เกิดข้อผิดพลาดในการบันทึก');
      }
          ...
          vaccine: {
            data: {
              ...
              attributes: {
                ...
                serviceStartTime: selectedVaccine?.serviceStartTime || initialData?.vaccine?.data?.attributes?.serviceStartTime,
                serviceEndTime: selectedVaccine?.serviceEndTime || initialData?.vaccine?.data?.attributes?.serviceEndTime,
              },
            },
          },
        },
      ...
      await MySwal.fire({
        title: 'สำเร็จ',
        text: 'บันทึกการแก้ไขช่วงเวลาให้บริการเรียบร้อยแล้ว',
        icon: 'success',
        customClass: {
          popup: 'bg-white/90 backdrop-blur-lg rounded-2xl shadow-lg border border-[#30266D]/50 p-6',
          title: 'text-lg font-bold text-[#30266D] mb-3',
      ...
      await MySwal.fire({
        title: 'เกิดข้อผิดพลาด',
        text: error.message === 'Authentication invalid or expired'
          ? 'กรุณาเข้าสู่ระบบใหม่'
          : error.message,
        icon: 'error',
        customClass: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 font-poppins p-6 bg-white/80 backdrop-blur-lg rounded-xl shadow-md border border-[#30266D]/50">
      <h3 className="text-xl font-semibold text-[#30266D]">แก้ไขช่วงเวลาให้บริการ</h3>
      
      <div>
        <label className="block mb-1 text-sm font-medium text-[#30266D]" htmlFor="vaccine-select">วัคซีน</label>
        {vaccines.length > 0 ? (
          <Select
            id="vaccine-select"
            options={vaccines}
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <label className="block mb-1 text-sm font-medium text-[#30266D]" htmlFor="start-time">เวลาเริ่ม</label>
          <Select
            id="start-time"
            options={timeOptions}
            value={startTime}
        ...
        <div className="flex-1">
          <label className="block mb-1 text-sm font-medium text-[#30266D]" htmlFor="end-time">เวลาสิ้นสุด</label>
          <Select
            id="end-time"
      ...
      <div>
        <label className="block mb-1 text-sm font-medium text-[#30266D]" htmlFor="quota">จำนวนโควต้า</label>
        <input
          id="quota"
          type="number"
          min={1}
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label className="text-sm font-medium text-[#30266D]" htmlFor="is-enabled">เปิดใช้งาน</label>
      </div>
      ...
      <div className="flex justify-end gap-2">
        <button
          ...
          aria-label="ยกเลิกการแก้ไขช่วงเวลาให้บริการ"
        >
          ยกเลิก
        </button>
        <button
          ...
          aria-label="บันทึกการแก้ไขช่วงเวลาให้บริการ"
        >
