...
export default function VaccineFormedit({ vaccine = {}, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (minAge < 0 || maxAge < minAge) {
      await MySwal.fire({
        ...
        text: 'กรุณาตรวจสอบช่วงอายุที่กรอก',
        customClass: {
          popup: 'bg-white rounded-xl shadow-lg border border-[#30266D] p-6',
          title: 'text-xl font-bold text-[#30266D] mb-3',
          htmlContainer: 'text-base text-[#30266D] font-medium mb-4',
    ...
    if (!maxQuota || maxQuota <= 0) {
      await MySwal.fire({
        icon: 'warning',
        title: 'ข้อมูลไม่ครบถ้วน',
        text: 'กรุณากรอกจำนวนสูงสุดที่รับได้',
        customClass: {
          popup: 'bg-white rounded-xl shadow-lg border border-[#30266D] p-6',
          title: 'text-xl font-bold text-[#30266D] mb-3',
          htmlContainer: 'text-base text-[#30266D] font-medium mb-4',
    ...
    if (dayjs(serviceEndTime.value, 'HH:mm').isBefore(dayjs(serviceStartTime.value, 'HH:mm'))) {
      await MySwal.fire({
        icon: 'warning',
        title: 'เวลาบริการไม่ถูกต้อง',
        text: 'เวลาสิ้นสุดต้องไม่ก่อนเวลาเริ่มต้น',
        customClass: {
          popup: 'bg-white rounded-xl shadow-lg border border-[#30266D] p-6',
          title: 'text-xl font-bold text-[#30266D] mb-3',
          htmlContainer: 'text-base text-[#30266D] font-medium mb-4',
    ...
    try {
      ...
      if (!resData?.data?.id) {
        throw new Error('ไม่พบข้อมูล ID จากการตอบกลับของ API');
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-lg border border-[#30266D]/50">
      <h3 className="text-xl font-semibold text-[#30266D]">{isEdit ? 'แก้ไขวัคซีน' : 'เพิ่มวัคซีนใหม่'}</h3>

      <div>
        <label className="block mb-1 font-medium text-[#30266D]">ชื่อวัคซีน *</label>
        <input
          type="text"
          value={title}
      ...
      <div>
        <label className="block mb-1 font-medium text-[#30266D]">คำอธิบาย</label>
        <textarea
          rows={3}
          value={description}
      ...
      <div className="flex gap-4">
          ...
          <Select
            ...
            styles={{
              control: (base) => ({
                ...base,
                backgroundColor: '#fff',
                borderColor: '#30266D4D',
                borderRadius: '0.75rem',
                padding: '0.5rem',
                boxShadow: 'none',
                '&:hover': { borderColor: '#F9669D' },
                '&:focus-within': { borderColor: '#F9669D', boxShadow: '0 0 0 2px #F9669D4D' },
              }),
              menu: (base) => ({
                ...base,
                backgroundColor: '#fff',
                border: '1px solid #30266D4D',
                borderRadius: '0.75rem',
                marginTop: '0.25rem',
              }),
              option: (base, state) => ({
        ...
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">ช่วงอายุต่ำสุด (ปี)</label>
          <input
            type="number"
            value={minAge}
        ...
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">ช่วงอายุสูงสุด (ปี)</label>
          <input
            type="number"
            value={maxAge}
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">วันที่เริ่มเปิดจอง *</label>
          <input
            type="date"
        ...
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">วันที่ปิดจอง *</label>
          <input
            type="date"
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">เวลาให้บริการ (เริ่ม)</label>
          <Select
        ...
        <div className="flex-1">
          <label className="block mb-1 font-medium text-[#30266D]">เวลาให้บริการ (สิ้นสุด)</label>
          <Select
      ...
      <div>
        <label className="block mb-1 font-medium text-[#30266D]">จำนวนสูงสุด *</label>
        <input
          type="number"
          value={maxQuota}
      ...
      <div className="flex items-center gap-4">
        <input
          ...
          onChange={() => setUseTimeSlots(!useTimeSlots)}
          className="w-4 h-4 text-[#F9669D] focus:ring-[#F9669D] border-[#30266D]/50 rounded"
        />
        <label htmlFor="useTimeSlots" className="font-medium text-[#30266D] select-none">ใช้ระบบช่วงเวลาการให้บริการ</label>
      </div>
      ...
      <div className="flex justify-end gap-3 mt-6">
        <button
          ...
          onClick={onCancel}
          disabled={submitting}
          className="px-4 py-2 bg-gray-300 text-[#30266D] rounded-xl font-semibold hover:bg-gray-400 transition-all duration-300 transform hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          ยกเลิก
        </button>
        <button
          type="submit"
          disabled={submitting}
          className="px-4 py-2 bg-[#F9669D] text-white rounded-xl font-semibold hover:bg-[#F9669D]/80 transition-all duration-300 transform hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed"
        >
